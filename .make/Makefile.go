...
lint: ## Run the golangci-lint application (install if not found)
	...
	@#Github Actions
	@if [ "$(shell command -v golangci-lint)" = "" ] && [ "$(GITHUB_WORKFLOW)" != "" ]; then curl -wget -O- -nv https://raw.githubusercontent.com/golangci/golangci-lint/master/install.sh | sh -s v1.33.0; fi;
	@#Brew - MacOS
